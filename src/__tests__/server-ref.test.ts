 ...
 * `await` gaps inside async tool handlers (e.g. after awaiting a
 * ConnectWise Automate API call, before sending an elicitation prompt back
 * through "the" server).
 *
...
describe("server-ref cross-tenant isolation", () => {
  ...
  it(
    ...
    async () => {
      ...
      // (simulating an in-flight ConnectWise Automate API call inside a
      // tool handler) BEFORE sending its elicitation/confirmation prompt.
